...
write.csv(calculated_combined,"D:/personal/UVA/Data analysis/202412 SILAC/FIG1. DDA and DIA/calculated_combined_DDA_DIA")
# ---- End of code block ----
...
create_color_palette <- function(ratio_hues, category_order) {
  ...
  # 定义亮度调整系数 - 使用更适中的值
  # 使用负值会让颜色变暗，正值会让颜色变亮
  brightness_adjust <- c(0, 0.35, 0.65, 0.95)  # 可以调整这些值
  ...
  for (ratio_name in names(ratio_hues)) {
    ...
    for (i in 1:length(category_order)) {
      ...
      if (i == 1) {
        # show_in_1_replicates 使用原始颜色
        palette[key] <- base_color
      } else {
        # 对于其他类别，使用colorspace的brighten函数
        # 这个函数保持色调和饱和度更稳定，只调整亮度
...
filtered_combined <- filtered_combined %>%
  rowwise() %>%
  mutate(
    Log10_Heavy_mean = log10(mean(c_across(c(PG.MS2Channel21, PG.MS2Channel22, PG.MS2Channel23)), na.rm = TRUE))
  ) %>%
  ungroup()
label_overlap_Heavy <- filtered_combined %>%
  filter(Acquisition == "DIA", !is.na(Log10_Heavy_mean)) %>%
  mutate(
    bin = cut(
      Log10_Heavy_mean,
      breaks = c(-Inf, seq(1, 6.8, by = 0.2), Inf),
...
bin_summary_Heavy <- label_overlap_Heavy %>%
  group_by(bin, R.Condition) %>%
  mutate(Total_Count = n()) %>%
  group_by(bin, R.Condition, DDA_overlap) %>%
  summarise(
...
bin_summary_Heavy <- bin_summary_Heavy %>%
  mutate(bin = as.character(bin))

lookup_Heavy <- bin_summary_Heavy %>%
  filter(!is.na(Label)) %>%     
  distinct(bin, Label)          
...
median_real_heavy <- label_overlap_Heavy %>%
  group_by(R.Condition, DDA_overlap) %>%
  summarise(
    median_log10 = median(Log10_Heavy_mean, na.rm = TRUE),
    .groups = "drop"
  ) %>%
  mutate(
    y = ifelse(DDA_overlap == "overlap", 1200, 1150),
...
ggplot(bin_summary_Heavy, aes(x = bin, y = Count, fill = DDA_overlap)) +
  geom_col(position = "stack", width = 0.9) +
  scale_y_continuous(
    breaks = c(300, 600, 900),
    limits = c(-50, 1200),   
  ...
  scale_x_discrete(
    breaks = lookup_Heavy$bin,    
    labels = lookup_Heavy$Label   
  ) +
  geom_text(
    data = median_real_heavy,
    aes(x = x, y = y, label = label),
    inherit.aes = FALSE,
    size = 2,
    hjust = median_real_heavy$hjust,
    vjust = median_real_heavy$vjust
  ) +
  labs(x="Log10(Heavy intensity)", y="Count", fill="DIA Overlap") +
  facet_wrap(~ R.Condition, ncol = 4, labeller = as_labeller(condition_labels)) +
  scale_fill_manual(
    values = c("non overlap" = "#009ade", "overlap" = "#C94D86"),
    labels = c("non overlap" = "DIA only", "overlap" = "Shared")
  )+
...
#DDA intensity based bin
#DDA Light
# ---- Start of code block ----
dda_bins <- filtered_combined %>%
  filter(Acquisition == "DDA", !is.na(Log10_Light_mean)) %>%
  mutate(
    bin = cut(Log10_Light_mean, breaks = c(-Inf, seq(6, 10, by = 0.2), Inf), include.lowest = TRUE)
  ) %>%
  select(PG.ProteinAccessions, R.Condition, bin)

label_overlap <- filtered_combined %>%
  filter(Acquisition == "DDA", !is.na(Log10_Light_mean)) %>%
  left_join(dda_bins, by = c("PG.ProteinAccessions", "R.Condition")) %>%
  mutate(
    DDA_overlap = case_when(
      R.Condition == "ratio1" & PG.ProteinAccessions %in% common_proteins_ratio1 ~ "overlap",
      R.Condition == "ratio3" & PG.ProteinAccessions %in% common_proteins_ratio3 ~ "overlap",
...
bin_summary <- label_overlap %>%
  ...
  mutate(
    Label = case_when(
      bin == "(6,6.2]" ~ "6", bin == "(7,7.2]" ~ "7", bin == "(8,8.2]" ~ "8",
      bin == "(9,9.2]" ~ "9", bin == "(10,10.2]" ~ "10",
      TRUE ~ NA_character_
    ),
    bin = as.character(bin)
  )

bin_bounds <- str_match(bin_summary$bin, "\\((.*?),(.*?)\\]")
bin_summary$bin_center <- rowMeans(apply(bin_bounds[, 2:3], 2, as.numeric), na.rm = TRUE)
...
median_dda_light <- label_overlap %>%
  group_by(R.Condition, DDA_overlap) %>%
  summarise(
    median_log10 = median(Log10_Light_mean, na.rm = TRUE),
    .groups = "drop"
  ...
  mutate(
    y = ifelse(DDA_overlap == "overlap", 400, 350),
    x = Inf,
    hjust = 1.1,
    vjust = 1.5,
    label = paste0("median (", DDA_overlap, ") = ", round(median_log10, 2))
  ...
  mutate(
    label = case_when(
      DDA_overlap == "non overlap" ~ paste0("Median (DDA only) = ", round(median_log10, 2)),
      DDA_overlap == "overlap"     ~ paste0("Median (Shared) = ", round(median_log10, 2)),
      TRUE                         ~ label
    )
  )
...
ggplot(bin_summary, aes(x = bin_center, y = Count, fill = DDA_overlap)) +
  geom_col(position = "stack", width = 0.18) +
  scale_y_continuous(
  ...
  scale_x_continuous(
    breaks = as.numeric(lookup$Label),   
    labels = lookup$Label,
    limits = c(6, 10)  
  ) +
  geom_text(
    data = median_dda_light,
    aes(x = x, y = y, label = label),
    inherit.aes = FALSE,
    size = 2,
    hjust = median_real$hjust,
  ...
  labs(x = "Log10(DDA Light intensity)", y = "Count", fill = "DDA Overlap") +
  facet_wrap(~ R.Condition, ncol = 4, labeller = as_labeller(condition_labels)) +
  scale_fill_manual(
    values = c("non overlap" = "#009ade", "overlap" = "#C94D86"),
    labels = c("non overlap" = "DDA only", "overlap" = "Shared")
  )+
  theme_minimal() +
  theme(
    panel.border = element_rect(color = "black", fill = NA, size = 0.75),
...
#DDA HEAVY
# ---- Start of code block ----
dda_bins_heavy <- filtered_combined %>%
  filter(Acquisition == "DDA", !is.na(Log10_Heavy_mean)) %>%
  mutate(
    bin = cut(Log10_Heavy_mean, breaks = c(-Inf, seq(6, 10, by = 0.2), Inf), include.lowest = TRUE)
  ) %>%
  select(PG.ProteinAccessions, R.Condition, bin)

label_overlap_heavy <- filtered_combined %>%
  filter(Acquisition == "DDA", !is.na(Log10_Heavy_mean)) %>%
  left_join(dda_bins_heavy, by = c("PG.ProteinAccessions", "R.Condition")) %>%
  mutate(
    DDA_overlap = case_when(
      R.Condition == "ratio1" & PG.ProteinAccessions %in% common_proteins_ratio1 ~ "overlap",
      R.Condition == "ratio3" & PG.ProteinAccessions %in% common_proteins_ratio3 ~ "overlap",
...
bin_bounds_heavy <- str_match(bin_summary_heavy$bin, "\\((.*?),(.*?)\\]")
bin_summary_heavy$bin_center <- rowMeans(apply(bin_bounds_heavy[, 2:3], 2, as.numeric), na.rm = TRUE)
...
median_dda_heavy <- label_overlap_heavy %>%
  group_by(R.Condition, DDA_overlap) %>%
  summarise(
    median_log10 = median(Log10_Heavy_mean, na.rm = TRUE),
    .groups = "drop"
...
ggplot(bin_summary_heavy, aes(x = bin_center, y = Count, fill = DDA_overlap)) +
  geom_col(position = "stack", width = 0.18) +
  scale_y_continuous(
    breaks = c(100, 200, 300,400),
    limits = c(-50, 500),
  ...
  scale_x_continuous(
    breaks = as.numeric(lookup_heavy$Label),  
    labels = lookup_heavy$Label,
    limits = c(6, 10)  
  ) +
  geom_text(
    data = median_dda_heavy,
    aes(x = x, y = y, label = label),
    inherit.aes = FALSE,
    size = 2,
    hjust = median_dda_heavy$hjust,
    vjust = median_dda_heavy$vjust
  ) +
  labs(x = "Log10(DDA Heavy intensity)", y = "Count", fill = "DDA Overlap") +
  facet_wrap(~ R.Condition, ncol = 4, labeller = as_labeller(condition_labels)) +
  scale_fill_manual(
    values = c("non overlap" = "#009ade", "overlap" = "#C94D86"),
    labels = c("non overlap" = "DDA only", "overlap" = "Shared")
...
plot_scatter_with_density(calculated_combined, acquisition = "DDA", intensity_type = "Heavy", intensity_mean_col = "Heavy_mean")

# DIA 
plot_scatter_with_density(calculated_combined, acquisition = "DIA", intensity_type = "Light")
plot_scatter_with_density(calculated_combined, acquisition = "DIA", intensity_type = "Heavy", intensity_mean_col = "Heavy_mean")

summarise_counts_direct <- function(df, acquisition_type, intensity_col, intensity_label) {
  df %>%
    filter(Acquisition == acquisition_type) %>%
    filter(!is.na(HtoL_ratio_mean), !is.na(.data[[intensity_col]])) %>%
    filter(log2(HtoL_ratio_mean) >= -4 & log2(HtoL_ratio_mean) <= 6.5) %>%
    mutate(
      R.Condition = factor(
        as.character(R.Condition),
        levels = c("ratio1", "ratio3", "ratio9", "ratio27"),
      ...
      log2_HL = log2(HtoL_ratio_mean),
      log2_Intensity = log2(.data[[intensity_col]])
    ) %>%
    group_by(R.Condition) %>%
    summarise(
      n = n(),
      HL_median = median(log2_HL, na.rm = TRUE),
      intensity_max = max(log2_Intensity, na.rm = TRUE),
      .groups = "drop"
    ) %>%
    mutate(
...
summary_DDA_Light <- summarise_counts_direct(calculated_combined, "DDA", "Light_mean", "Light")
summary_DIA_Light <- summarise_counts_direct(calculated_combined, "DIA", "Light_mean", "Light")
summary_DDA_Heavy <- summarise_counts_direct(calculated_combined, "DDA", "Heavy_mean", "Heavy")
summary_DIA_Heavy <- summarise_counts_direct(calculated_combined, "DIA", "Heavy_mean", "Heavy")

full_summary <- bind_rows(summary_DDA_Light, summary_DIA_Light, summary_DDA_Heavy, summary_DIA_Heavy) %>%
  relocate(Acquisition, Intensity, R.Condition)
print(full_summary)
...
plot_scatter_with_density(processed_combined_cv20, acquisition = "DDA", intensity_type = "Heavy", intensity_mean_col = "Heavy_mean")

# DIA 
plot_scatter_with_density(processed_combined_cv20, acquisition = "DIA", intensity_type = "Light")
plot_scatter_with_density(processed_combined_cv20, acquisition = "DIA", intensity_type = "Heavy", intensity_mean_col = "Heavy_mean")
...
summary_DDA_Light <- summarise_counts_direct(processed_combined_cv20, "DDA", "Light_mean", "Light")
summary_DIA_Light <- summarise_counts_direct(processed_combined_cv20, "DIA", "Light_mean", "Light")
summary_DDA_Heavy <- summarise_counts_direct(processed_combined_cv20, "DDA", "Heavy_mean", "Heavy")
summary_DIA_Heavy <- summarise_counts_direct(processed_combined_cv20, "DIA", "Heavy_mean", "Heavy")
